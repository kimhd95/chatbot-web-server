/* jshint node: true, devel: true */
'use strict';

const info = require('../../info_update'),
      index = require('../../../server/index.js'),
      info_update = new info(),
      sortMap = require('sort-map');

let lat, lng, cnt;
let mid_lat, mid_lng;

let dictionary = {
    "station":[
        {"name":"홍대입구역","lat":"37.557527","lng":"126.9244669"},
        {"name":"합정역","lat":"37.5495753","lng":"126.9139908"},
        {"name":"신촌역","lat":"37.559768","lng":"126.942308"},
        {"name":"망원역","lat":"37.5559772","lng":"126.9012821"},
        {"name":"서울대입구역","lat":"37.48121","lng":"126.952712"},
        {"name":"왕십리역","lat":"37.5611284","lng":"127.035505"},
        {"name":"성수역","lat":"37.544569","lng":"127.055974"},
        {"name":"건대입구역","lat":"37.540389","lng":"127.069236"},
        {"name":"이태원역","lat":"37.534533","lng":"126.994579"},
        {"name":"명동역","lat":"37.5609892","lng":"126.9861868"},
        {"name":"잠실역","lat":"37.5132612","lng":"127.1001336"},
        {"name":"강남역","lat":"37.49794199999999","lng":"127.027621"},
        {"name":"사당역","lat":"37.476559","lng":"126.981633"},
        {"name":"신사역","lat":"37.524124","lng":"127.022883"},
        {"name":"고속터미널역","lat":"37.5049142","lng":"127.0049151"},
        {"name":"회기역","lat":"37.58975600000001","lng":"127.057977"},
        {"name":"안암역","lat":"37.5858384","lng":"127.0213534"},
        {"name":"혜화역","lat":"37.58208","lng":"127.001892"},
        {"name":"여의도역","lat":"37.5215695","lng":"126.9243115"},
        {"name":"종각역","lat":"37.570169","lng":"126.983099"},
        {"name":"종로3가역","lat":"37.5715","lng":"126.9912475"},
        {"name":"을지로입구역","lat":"37.566056","lng":"126.982662"},
        {"name":"을지로3가역","lat":"37.566286","lng":"126.9917735"},
        {"name":"안국역","lat":"37.576556","lng":"126.985472"},
        {"name":"광화문역","lat":"37.5705263","lng":"126.9765729"}
    ]
};
let data = dictionary.station;

function distance(lat1, lon1, lat2, lon2, value) { //Haversine 공식 : 구(지구) 에서 두 점(좌표) 사이 최단거리를 구하는 공식
  var p = 0.017453292519943295;    // Math.PI / 180
  var c = Math.cos;
  var a = 0.5 - c((lat2 - lat1) * p)/2 +
          c(lat1 * p) * c(lat2 * p) *
          (1 - c((lon2 - lon1) * p))/2;
  var result = 12742 * Math.asin(Math.sqrt(a));
  // map.set(result, value);
  return [result,value];// 2 * R; R = 6371 km
}

exports.decide_place = function(val, socket, user_data) {
       if (val === 'decide_place'){

        info_update.profile.update_state(socket.id, '2', 'init', function (err) {
            if(err){
                console.log("에러 : " + err);
                return err;
            }else{
            return;
          }
        });
        info_update.profile.update_place_start(user_data.kakao_id, function (error) {
            if(error){
                console.log("에러 : " + error);
                return error;
            }
            return;
        });
        index.sendSocketMessage(socket.id, 'chat message', "너의 출발위치를 입력해줘!");
      } else if (val === '다른 친구도 있어'){

        info_update.profile.update_state(socket.id, '2', 'init', function (err) {
            if(err){
                console.log("에러 : " + err);
                return err;
            }else{
            return;
          }
        });
        index.sendSocketMessage(socket.id, 'chat message', "친구"+ (parseInt(user_data.cnt)) +"의 출발위치를 입력해줘ㅎㅎ");
      } else if (val === '응 다야'){
        info_update.profile.update_mid_info(user_data.kakao_id, (user_data.lat/user_data.cnt), (user_data.lng/user_data.cnt), function (error) {
              if(error){
                  return error;
              }
              return;
        });
        index.sendSocketMessage(socket.id, 'chat message button', user_data.cnt+'곳의 중간, 핫-플레이스 보여줄게맨(BAAAM)', ['볼래!', '볼래!'],['get_started', '취소취소']);
      } else if (val === '볼래!'){
        let map = new Map();

        for(var i in data)
        {
             map.set(distance(parseFloat(user_data.mid_lat),parseFloat(user_data.mid_lng),data[i].lat, data[i].lng, data[i].name)[0], distance(parseFloat(user_data.mid_lat),parseFloat(user_data.mid_lng),data[i].lat, data[i].lng, data[i].name)[1]);
        }

        let sortedMap = sortMap(map); //key-value형식으로 map이 정의되어있다 (ex. 0.1234 - 성수역) 오름차순으로 map을 정렬하는 module
        console.log("거리순으로 정렬된 map : " + sortedMap); // 거리순으로 출력

        info_update.profile.update_state(socket.id, '1', 'decide_menu', function (err) {
            if(err){
                console.log("에러 : " + err);
                return err;
            }else{
            return;
          }
        });
        index.sendSocketMessage(socket.id, 'chat message button', '중간 지점에 있는 주요 역은\n'+Array.from(sortedMap.values())[0]+'\n'+Array.from(sortedMap.values())[1]+'\n'+Array.from(sortedMap.values())[2]+'\n이야!',
        [Array.from(sortedMap.values())[0], Array.from(sortedMap.values())[0]+' 맛집 보러가기!'],[Array.from(sortedMap.values())[1], Array.from(sortedMap.values())[1]+' 맛집 보러가기!'],
        [Array.from(sortedMap.values())[2], Array.from(sortedMap.values())[2]+' 맛집 보러가기!'],['get_started', '처음으로']);
      }

      if(user_data.state === 'init'){
        info_update.profile.geocoder(user_data.kakao_id, val, function(error,body){
          if (error) {
              console.log(error);
              return;
          } else {
              let re = JSON.parse(body);
              // console.log(re.documents);
              console.log(re.documents[0]);
              if(re.documents[0] === undefined){
                index.sendSocketMessage(socket.id, 'chat message', val + "가 어딘지 모르겠어 ㅠㅠ 다른 곳으로 입력해줄래?\n(근처 지하철역을 말해주면 찾기가 쉬워)");
              }else{
              info_update.profile.update_place_info(socket.id, (user_data.lat+parseFloat(re.documents[0].y)), (user_data.lng+parseFloat(re.documents[0].x)), (user_data.cnt+1), function (error) {
                    if(error){
                        return error;
                    }
                    return;
              });
              if(parseInt(user_data.cnt) >= 1){
                info_update.profile.update_state(socket.id, '2', 'reinit', function (err) {
                    if(err){
                        console.log("에러 : " + err);
                        return err;
                    }else{
                    return;
                  }
                });
                index.sendSocketMessage(socket.id, 'chat message button', (parseInt(user_data.cnt)) +"명이 다야?", ['응 다야', '응 다야'],['다른 친구도 있어','다른 친구도 있어']);
              }else{
              index.sendSocketMessage(socket.id, 'chat message', "친구"+ (parseInt(user_data.cnt)+1) +"의 출발위치를 입력해줘ㅎㅎ");
              }
              return;
            }
          }
        });
      }
      return;
};
