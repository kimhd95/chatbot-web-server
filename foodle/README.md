...
사용자의 여러 선택을 반영하기 위해 만든 형식이다. 식당 분위기를 **약속** 으로 선택했을 때 나오는 메세지 형식이인데, 이 형식은 버튼을 눌러도 다음시나리오가 진행되지 않고, 체크만 된다. 해당 버튼을 체크하고, 선택완료 버튼을 누르게 되면, 체크된 값을 판단해서 시나리오에 반영시킨다.
