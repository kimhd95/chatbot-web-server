    ...
    (async function () {
      try {
        ...
        if (result === 'success') {
          ...
          index.sendSocketMessage(socket.id, 'chat message button', `지금 카페 고르기를 이용 가능한 곳은 서울[${subways}]이야. 다른 곳 식당도 열심히 가서 먹어보고 곧 알려줄게!`, ['decide_cafe', '다시 장소 입력하기'], ['get_started', '처음으로 돌아가기']);
        }
      } catch (e) {
        index.sendSocketMessage(socket.id, 'chat message button', '오류가 발생했습니다.', ['get_started', '처음으로 돌아가기']);
        console.log(e);
    ...
    (async function () {
      try {
        ...
        if(cafe_value[0].holiday != null && cafe_value[0].holiday != '') {
          component.push(`휴무일 : ${cafe_value[0].holiday}<br>`);
        }
