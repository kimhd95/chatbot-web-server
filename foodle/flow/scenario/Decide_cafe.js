...
class Decide_cafe {
  constructor(value, socket, user_data) {
    ...
    this.strategies = {
      ...
      'no_cafe_type': this.no_cafe_type,
  ...
  cafe_type(value, socket, user_data) {
    (async function () {
      try {
        ...
        const chlist = ['지금 고르는 카페에서 가장 우선순위는?'];
        const leng = chlist.length;
        const rand = Math.floor(leng * Math.random());
      //  let cafe_type = await info_update.drink.find_subway_drink_type(socket.id, user_data.subway, user_quarter);
        index.sendSocketMessage(socket.id, 'chat message button', chlist[rand], ['quality/cafe_type', '메뉴의 퀄리티'], ['comfort/cafe_type', '편하게 대화 가능한 곳'], ['meeting/cafe_type','독서와 과제, 비즈니스 미팅이 가능한'], ['meal/cafe_type','끼니 해결'] ['experience/cafe_type','이색 체험']);
