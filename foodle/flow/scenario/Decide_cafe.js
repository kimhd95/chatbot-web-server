...
scenario10_5 (value, socket, user_data) {
  ...
  //
  //         // 테마* 전체 카페 리스트
  //         let cafe_thema = await info_update.cafe.verify_subway_thema(socket.id, user_data.subway_cafe);
  //         const cafe_thema_array = await cafe_thema.result_array;
  //         const themaRand = await Math.floor(Math.random() * cafe_thema_array.length);
  //
  //         const mainmenu_type = await value.split('/')[1];
  //         await info_update.profile.update_mainmenu_type(socket.id, mainmenu_type);
  //         const cafe_rest1_result = await info_update.cafe.get_cafe(socket.id, user_data.subway_cafe, user_data.exit_quarter, mainmenu_type);
  //         console.log(cafe_rest1_result);
  //         // const cafe_rest1 = await cafe_rest1_result.message;
  //         // if (mainmenu_type.includes('!')) {
  //         //   const conditionList = await info_update.cafe.verify_subway_detail_thema(socket.id, user_data.subway_cafe, "테마(보드게임)");
  //         //   const filter_list = await conditionList.result_array;
  //         //   // let filtered_list = await cafe_thema_array.filter(function(e1) {
  //         //   //   return !filter_list.includes(e1);
  //         //   // });
  //         //   // console.log(filtered_list);
  //         //   for(var i = cafe_thema_array.length - 1; i>=0; i--){
  //         //    	for( var j=0; j<filter_list.length; j++){
  //         //    	    if(cafe_thema_array[i] && (cafe_thema_array[i].name === filter_list[j].name)){
  //         //       		cafe_thema_array.splice(i, 1);
  //         //       	}
  //         //       }
  //         //   }
  //         //   console.log(cafe_thema_array);
  //         // } else {
  //
  //         // }
  //
  //         // cafe_rest1와 cafe_rest2 중복 피하기 위함 -> 이부분 비동기 처리 필요
  //         var cafe_rest2 = await cafe_thema_array[themaRand];
  //         if(cafe_thema_array[themaRand].id === cafe_rest1.id){
  //           cafe_rest2 = await cafe_thema_array[themaRand - 1];
  //         }
  //
  //         // 테마, 출구까지 맞는 cafe
  //         if(cafe_rest1_result.exit_quarter){
  //           await info_update.profile.update_limit_cnt_cafe(socket.id, user_data.limit_cnt_cafe + 1);
  //           await info_update.profile.update_cafe2(user_data.kakao_id, cafe_rest1.id, cafe_rest2.id);
  //           await index.sendSocketMessage(socket.id, 'chat message button', '오키 잘알겠어~ 2곳을 골라줄테니까 한 번 골라봐!');
  //           await index.sendSocketMessage(socket.id, 'chat message loader', 2000);
  //           await index.sendSocketMessage(socket.id, 'chat message button', chlist[rand]);
  //           await index.sendSocketMessage(socket.id, 'chat message loader', 2500);
  //           const first_url = `https://search.naver.com/search.naver?where=nexearch&sm=top_hty&fbm=1&ie=utf8&query=${cafe_rest1.subway} ${cafe_rest1.cafe_name}`;
  //           const second_url = `https://search.naver.com/search.naver?where=nexearch&sm=top_hty&fbm=1&ie=utf8&query=${cafe_rest2.subway} ${cafe_rest2.cafe_name}`;
  //           const first_map_url = `https://map.naver.com/index.nhn?query=${cafe_rest1.subway} ${cafe_rest1.cafe_name}&tab=1`;
  //           const second_map_url = `https://map.naver.com/index.nhn?query=${cafe_rest2.subway} ${cafe_rest2.cafe_name}&tab=1`;
  //           const image = await info_update.food.crawl_two_image(socket.id, `${cafe_rest1.subway.slice(0, -1)} ${cafe_rest1.cafe_name}`, `${cafe_rest2.subway.slice(0, -1)} ${cafe_rest2.cafe_name}`);
  //           if (image.res1 === 'no image') {
  //             await index.sendSocketMessage(socket.id, 'chat message card no image', ['final/1', cafe_rest1.cafe_name], ['final/2',cafe_rest2.cafe_name], ['final/3', '챗봇이 골라주기'], [cafe_rest1.cafe_name, cafe_rest1.mainmenu_type, cafe_rest1.drink_name, first_url, first_map_url], [cafe_rest2.cafe_name, cafe_rest2.mainmenu_type, cafe_rest2.drink_name, second_url, second_map_url]);
  //           } else {
  //             await index.sendSocketMessage(socket.id, 'chat message card', ['final/1', cafe_rest1.cafe_name], ['final/2', cafe_rest2.cafe_name], ['final/3', '챗봇이 골라주기'], [cafe_rest1.cafe_name, cafe_rest1.mainmenu_type, cafe_rest1.drink_name, first_url, first_map_url, image.res1[0], image.res1[1], image.res1[2]], [cafe_rest2.cafe_name, cafe_rest2.mainmenu_type, cafe_rest2.drink_name, second_url, second_map_url, image.res2[0], image.res2[1], image.res2[2]]);
  //           }
  //         } else if(!cafe_rest1_result.exit_quarter){
  //           await info_update.profile.update_limit_cnt_cafe(socket.id, user_data.limit_cnt_cafe + 1);
  //           await info_update.profile.update_cafe2(user_data.kakao_id, cafe_rest1.id, cafe_rest2.id);
  //           await index.sendSocketMessage(socket.id, 'chat message button', `조건에 맞는 이색카페가 해당 출구에 없어ㅠㅠ 다른 출구에서 보여줄게!`);
  //           await index.sendSocketMessage(socket.id, 'chat message loader', 2000);
  //           await index.sendSocketMessage(socket.id, 'chat message button', chlist[rand]);
  //           await index.sendSocketMessage(socket.id, 'chat message loader', 2500);
  //           const first_url = `https://search.naver.com/search.naver?where=nexearch&sm=top_hty&fbm=1&ie=utf8&query=${cafe_rest1.subway} ${cafe_rest1.cafe_name}`;
