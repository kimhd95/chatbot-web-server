    ...
    (async function () {
      try {
        ...
        if (user_info.registered == -1) {
            ...
            index.sendSocketMessage(socket.id, 'chat message button image', `${chlist[rand]}`, `${imglist[rand2]}`,['decide_menu/lunch', '점심 고르기(평일)'],['decide_menu/dinner', '점심 고르기(주말)'],  ['decide_menu/dinner', '저녁 고르기'],['hobulho_hate_start', '못 먹는 음식 체크'], ['search_near', '내 주변 500m 내 식당 검색(GPS 켜줘!)']); //TODO: 식당/메뉴명으로 검색하기
        } else {
          if (result === 'success') {
            await info_update.profile.update_stack(socket.id, `{"state": "${user_data.state}", "value": "${value}"}`);
            const rand = Math.floor(chlist.length * Math.random());
  ...
  hobulho_hate(value, socket, user_data) { //todo: api 구현
    (async function () {
        try {
            ...
            const hobulho_hate_rand = Math.floor(hobulho_hate_list.length * Math.random());
  ...
  hobulho_hate_feedback(value, socket, user_data) {
    (async function () {
        try {
            ...
            const hobulho_hate_feedback_rand = Math.floor(hobulho_hate_feedback_list.length * Math.random());
            index.sendSocketMessage(socket.id, 'chat message button', `${hobulho_hate_feedback_list[hobulho_hate_feedback_rand]}`,['decide_menu/lunch', '점심 고르기(평일)'],['decide_menu/dinner', '점심 고르기(주말)'],  ['decide_menu/dinner', '저녁 고르기'],['hobulho_hate_start', '못 먹는 음식 체크'], ['search_near', '내 주변 500m 내 식당 검색(GPS 켜줘!)']);
        } catch (e) {
            index.sendSocketMessage(socket.id, 'chat message button', '오류가 발생했습니다.', ['get_started', '처음으로 돌아가기']);
            console.log(e);
        }
    ...
    (async function () {
        try {
          ...
          if (value.includes('exit/')) {
            ...
            const price_rand = Math.floor(price_list.length * Math.random());
  ...
  location(value, socket, user_data) {
      (async function () {
          try {
              ...
              const location_rand = Math.floor(location_list.length * Math.random());
  ...
  decide_subway_corgi(value, socket, user_data) {
    (async function () {
        try {
            ...
            const subway_db_rand = Math.floor(subway_db.length * Math.random());
    ...
    (async function () {
        try {
            if (!value.includes('elsewhere') && user_data.freq_subway !== null) {
                ...
                const freq_rand = Math.floor(freq_list.length * Math.random());
          ...
          try {
              ...
              if (!value.includes('elsewhere') && user_data.freq_subway !== null) {
                  ...
                  const freq_list = [`이번에도 ${revisit}에서 메뉴를 정하면 될까?`, `이번에도 ${revisit} 고고?`, `이번에도 ${revisit}에서 밥 먹을거야?`,
                      ...
                      `이번에도 ${revisit}에서 정하는거 맞아맞아?`, `오늘도 ${revisit}에서 메뉴 정해볼까?`, `이번에도 ${revisit}에서 먹을 곳 찾는거야?`];
                  const freq_rand = Math.floor(freq_list.length * Math.random());
                  index.sendSocketMessage(socket.id, 'chat message button', freq_list[freq_rand], [`${revisit}`, '응 맞아!'], ['decide_subway/elsewhere', '다른 곳이야!'], ['previous/' + user_data.stack.replace(/"/gi, "@"), '이전으로 돌아가기']);
              } else {
                  const chlist = ['어느 역 근처의 메뉴를 정해줄까?', '밥 어디에서 먹을거야?🍚', '밥 어디에서 먹어?', '어느 역 근처 메뉴를 정해줄까?',
                      '위치가 어디야? 원하는 곳에서 가까운 지하철역을 입력해줘🚋', '밥 어디에서 먹어? 챱챱', '이번에는 어느 역 근처의 메뉴를 정해볼까?',
                      '어디 근처로 골라볼까?', '어떤 지하철역 근처로 정해볼까?'];
    ...
    (async function () {
      try {
        const mood_list = ['메뉴 컨셉은 끼니해결? 아니면 약속 자리?',
          ...
          '어떤 자리에 맞는 메뉴를 골라줄까?'];
        const mood_rand = Math.floor(mood_list.length * Math.random());
  ...
  mood2(value, socket, user_data) {
    (async function () {
      try {
        ...
        const mood2_rand = Math.floor(mood2_list.length * Math.random());
  ...
  food_type(value, socket, user_data) {
    (async function () {
      try {
        ...
        const type_rand = Math.floor(type_data.length * Math.random());
        ...
        const rand2 = Math.floor(chooseimglist.length * Math.random());
          ...
          // if (moment().format('HH') >= 10 && moment().format('HH') <= 15 && food_value[0].lunch_option === 1) {
            if(result.success) {
              ...
              index.sendSocketMessage(socket.id, 'chat message button image', `오늘의 선택: ${food_value[0].res_name}<br>${food_value[0].subway}에 있는 ${food_value[0].food_name} 전문 ${food_value[0].food_type}집이야!<br>(런치메뉴 있음)`
                + `<hr class="link-line"><a href="${map_url}" target="_blank" class="card-link" style="bottom:8%;"><i class="fas fa-map-marked-alt link-icon" style="margin-right: 4px;"></i>지도 보기</a><br><a href="${naver_url}" target="_blank" class="card-link"><i class="fas fa-link link-icon"></i>네이버 검색 결과</a><br><a class="card-link" target="_self" href="#" onclick="location.href='tel:${food_value[0].phone}';"><i class="fa fa-phone"></i> 전화 걸기</a>`,
                  `${chooseimglist[rand2]}`,['show_image', '사진 보기'], ['decide_final', '뒤로가기'], ['get_started', '처음으로 돌아가기']);
            }
          } else {
            if(result.success) {
              index.sendSocketMessage(socket.id, 'chat message button image', `오늘의 선택: ${food_value[0].res_name}<br>${food_value[0].subway}에 있는 ${food_value[0].food_name} 전문 ${food_value[0].food_type}집이야!`
