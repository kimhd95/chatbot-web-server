    ...
    try {
      ...
      if (val === 'get_started') {
        ...
        await info_update.profile.update_state(socket.id, '100', 'init');
          ...
          index.sendSocketMessage(socket.id, 'chat message button', chlist2[rand2], ['decide_menu', '식당 고르기'], ['decide_drink', '술집 고르기'], ['decide_cafe', '카페 고르기'], ['user_feedback', '개발팀에게 피드백하기'], ['chitchat', '외식코기랑 대화하기']);
      } else {
